...
public class Menu {
    ...
    public void create(Player p, int slots, @NotNull InventoryType type) {
        switch (type) {
            case DEVELOP_BIZ : {
                ...
                setItems(p, developBiz, type, ItemType.DEFAULT_ITEM, ItemType.HIGH_CLASS, ItemType.MEDIUM_CLASS, ItemType.LOW_CLASS);
    ...
    public void setItems(Player p, Inventory inventory, @NotNull InventoryType type, ItemType... item) {
        ...
        switch (type) {
            ...
            case DEVELOP_BIZ : {
                for(int i : handlerBusiness.getAllHaveBiz(p)) {
                    for(int slots = 0; slots < inventory.getSize(); i++) {
                        if(inventory.getItem(slots) != null) {
                            i++;
                            continue;
                        }
                        inventory.setItem(slots, items.setItemMeta(new ItemStack(Material.getMaterial(getString("Business.Id." + i + ".Item"))),
                                getStringList("Business.Id." + i + ".Lore"), getString("Business.Id." + i + ".Name")));
                    }
                }
            }
