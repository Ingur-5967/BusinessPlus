    ...
    public void setItems(Player p, Inventory inventory, @NotNull InventoryType type, ItemType... item) {
        ...
        switch (type) {
            ...
            case CHANGE_UPDATE: {
                ...
                int biz = data.getInt("players." + p.getUniqueId() + ".OtherInfo");

                items.setItemInv(item[0].getItemStack(), inventory, 0, 36);

                inventory.setItem(15, items.setItemMeta(new ItemStack(Material.getMaterial(getString("Business.Id." + biz + ".Item"))), getStringList("Business.Id." + biz + ".Lore"),
                        getString("Business.Id." + biz + ".Name")));

                inventory.setItem(21, items.setItemMeta(new ItemStack(Material.getMaterial(custom.getString("Menu.ChangeUpdate.HalfTax.Item"))), custom.getList("Menu.ChangeUpdate.HalfTax.Lore"), custom.getString("Menu.ChangeUpdate.HalfTax.Name")));
                inventory.setItem(22, items.setItemMeta(new ItemStack(Material.getMaterial(custom.getString("Menu.ChangeUpdate.Staff.Item"))), custom.getList("Menu.ChangeUpdate.Staff.Lore"), custom.getString("Menu.ChangeUpdate.Staff.Name")));

                if(data.getBoolean("players." + p.getUniqueId() + ".isPremium")) {
                    inventory.setItem(23, items.setItemMeta(new ItemStack(Material.getMaterial(custom.getString("Menu.ChangeUpdate.X2.Item"))), custom.getList("Menu.ChangeUpdate.X2.Lore"), custom.getString("Menu.ChangeUpdate.X2.Name")));
                } else {
                    inventory.setItem(23, items.setItemMeta(ItemType.DENY_SLOT.getItemStack(), custom.getList("Menu.ChangeUpdate.X2.DenyLore"), custom.getString("Menu.ChangeUpdate.X2.Name")));
                }
                return;
            }
