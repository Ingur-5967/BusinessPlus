...
public class HandlerBusiness  {
    ...
    public boolean isSizeMoreLimit(@NotNull BusinessClassType classType) {
        switch (classType) {
            ...
            case HIGH: {
                ...
                if (sizeClassBiz > 20) {
                    Main.getInstance().getLogger().warning("You have exceeded the number limit in the class <" + classType.name() + ">. Size: " + sizeClassBiz + ". Limit: 20" );
                    return false;
                }
                return true;
            }
    ...
    public int getBizId(String name) {
        ...
        Main.getInstance().getLogger().warning("Couldn't find a business with the name '" + name + "'! Maybe you didn't write it in 'RegistrationId'");
        return -1;
    }
