...
public class HandlerBusiness  {
    ...
    public List<Integer> getAllHaveBiz(Player p) {
        for(int i : getIntegerList("Business.RegistrationId")) {
            if(new CustomConfig("data").getString("players." + p.getUniqueId() + ".Business." + i) == null) continue;
            business.add(i);
        }
        return business;
    }
    ...
    public boolean isHaveBusiness(@NotNull Player p, int id) {
        return new CustomConfig("data").getString("players." + p.getUniqueId() + ".Business." + id) != null;
    }
}